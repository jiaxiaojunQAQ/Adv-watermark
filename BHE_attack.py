...
def object_function(path,label,sl,xs):
    img = Image.open(path)
    attack_image = add_watermark_to_image(img, xs, watermark, sl)
    attack_image = attack_image.convert('RGB')
    predict = label_model(model, attack_image).cpu().detach().numpy()
    print(predict[0][int(label)])
    return predict[0][int(label)]
...
def initpara():
    ...
    len_x = 3
    sl=4
    watermark_logo='Berkely'
    watermark = Image.open("Berkely.png")
    niter=3
    ...
    value_up_range = [255, 224-watermark_x1, 224-watermark_y1]
    value_down_range = [100,0,0]
    return NP, F, CR, generation, len_x, value_up_range, value_down_range,sl,watermark,watermark_logo,niter
...
## BH 变异
def mutation(model,img,label,watermark,sl,np_list):
    v_list = []
    for i in range(0, NP):


        x_list, _= BH_Calculation(model,img, label, watermark, sl, xs=np_list[i],niter=niter)
...
# 选择
def selection(path,label,sl,u_list, np_list):
    for i in range(0, NP):
        if object_function(path,label,sl,u_list[i]) <= object_function(path,label,sl,np_list[i]):
            np_list[i] = u_list[i]
        else:
            np_list[i] = np_list[i]
    return np_list
...
# 主函数
NP, F, CR, generation, len_x, value_up_range, value_down_range,sl,watermark,watermark_logo,niter= initpara()

def BHE_Attack(path,label,np_list):
    ...
    for ii in range(0, NP):
        flag=False
        flag=decide_function(path,label,sl,np_list[ii])
        if flag==True:
            image=Image.open(path)
            result_img=add_watermark_to_image(image, np_list[ii], watermark, sl)
            mkdir('BHE_logo/scale_' + str(sl)+'/'+watermark_logo+'/imagenet')
            result_img.save('BHE_logo/scale_' + str(sl) +'/'+watermark_logo+ '/'+path)
            return True
        xx.append(object_function(path,label,sl,np_list[ii]))
    ...
    for i in range(0, generation):
        print('第'+str(i+1)+'代')
        img=Image.open(path)
        v_list = mutation(model,img,label,watermark,sl,np_list)
        u_list = crossover(np_list, v_list)
        np_list = selection(path,label,sl,u_list, np_list)

        xx = []
        for j in range(0, NP):
            flag = False
            flag = decide_function(path, label, sl, np_list[j])
            print(flag)
            if flag == True:
                image = Image.open(path)
                result_img = add_watermark_to_image(image, np_list[j], watermark, sl)
                mkdir('BHE_logo/scale_' + str(sl) + '/' + watermark_logo + '/imagenet')
                result_img.save('BHE_logo/scale_' + str(sl) + '/' + watermark_logo + '/' + path)
                return  True
            xx.append(object_function(path,label,sl,np_list[j]))
        # print('!!!!!!!')
        # print(xx)
        # print(min(xx))
        # print("%%%%%")
        ...
        min_x.append(np_list[xx.index(min(xx))])
        image = Image.open(path)
        result_img = add_watermark_to_image(image, np_list[i], watermark, sl)
        mkdir('BHE_logo/scale_' + str(sl) + '/' + watermark_logo + '/imagenet')
        result_img.save('BHE_logo/scale_' + str(sl) + '/' + watermark_logo + '/' + path)
    return False
count=0
...
for i in range(1000):
    ...
    predict = label_model(model,img).cpu().detach().numpy()
    # print(np.argmax(predict))
    if np.argmax(predict) == int(classes[i]):
        np_list = initialtion(NP)
        count=count+1
        result_flag=False
        result_flag=BHE_Attack(path,classes[i],np_list)
        if result_flag==True:
            count1=count1+1
print(watermark_logo,sl)
print('攻击之前：',count)
print('攻击之后：',count-count1)
print('攻击成功率：',count1/count)
